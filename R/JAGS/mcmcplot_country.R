mcmcplot_country <- function (mcmcout, parms = NULL, regex = NULL, random = NULL, 
          leaf.marker = "[\\[_]", dir = tempdir(), filename = "MCMCoutput", 
          extension = "html", title = NULL, heading = title, 
          col = NULL, lty = 1, xlim = NULL, ylim = NULL, style = c("gray","plain"),
          greek = FALSE,
          country = NULL,
          type = NULL) 
{
  ...
  htmlfile <- .html.begin(dir, filename, extension, title = title, 
                          cssfile = css.file)
  ...
  for (group.name in names(parnames)) {
    ...
    for (p in parnames[[group.name]]) {
      ...
      gname <- paste(country,'_',type,'_', p, ".png", sep = "")
      png(file.path(dir, gname), width = htmlwidth, height = htmlheight)
      plot_err <- tryCatch({
        mcmcplot1(mcmcout[, p, drop = FALSE], col = col, 
      ...
      if (inherits(plot_err, "error")) {
        ...
        .html.img(file = htmlfile, class = "mcmcplot", 
                  src = gname, width = htmlwidth, height = htmlheight)
      }
    }
  }
  cat("\r", rep(" ", getOption("width")), 
...
environment(mcmcplot_country) <- environment(mcmcplot)
