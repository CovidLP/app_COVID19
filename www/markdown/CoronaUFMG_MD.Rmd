...
onde $Y(t)$ é o **número de casos acumulados** até o dia $t$ em uma dada região, com $\mu ( t ) = \frac{ a .\, \exp{ \{ c t \} } } {1 + b .\, \exp { \{ c t \} }}.$


**Caso especial:** 

- $b = 0$ (crescimento exponencial) $\rightarrow \mu(t) = a .\,\exp \{ ct \}$;
...
a) os dados são **contagens**, e a distribuição normal pressupõe dados contínuos;
...
- Reflete o total de casos acumulados ao longo de toda a pandemia.
...
* $Var[Z] = Var[ E(Z \mid W ) ] +  E[ Var( Z \mid W ) ]$
  ...
  Em particular, $E[Y ( t + k ) \mid Y( 1:t)] = E\{ E[ Y ( t + k ) \mid \mu( 1:t)] \mid Y( 1:t) \} = E[ \mu( t+k )] \mid Y( 1:t) ]$, a média a posteriori de $\mu ( t + k )$.
...
No tempo $t$, ele é definido como $R_0 = \frac {\mu ( t ) - \mu ( t-1)}{\mu ( t-1)}  = \frac {\mu ( t )}{\mu ( t-1)} - 1$.

- **Início da epidemia:** $1 \gg b \exp { \{ ct \} } \to \mu (t) \approx a \exp { \{ ct \} } \to R_0 \approx e^c - 1$

- **Final da epidemia:** $1 \ll b \exp { \{ ct \} } \to \mu (t) \approx a / b \to R_0 \approx 0$

- **Meio da epidemia:** $R_0$ é função dos parâmetros $(a,b,c)$ e do tempo $t$, e é dado por $R_0 (t) = e^c \ \frac{1 + b e^c e^{ct} } {1 + b e^{ct} } \ - \ 1$.
...
Número médio de novos casos no tempo $t+k$:
...
Estimativas e Intervalo de Credibilidade para o pico da pandemia e para o total de casos
...
Modelos anteriores assumem comportamento estático:

- a forma da doença não se modifica ao longo do tempo;

- taxa de infecção será sempre a mesma, assintota será sempre a mesma, \...
...
<br>
##### 3. **Evolução multiplicativa com erros normais**
...
Até agora, usamos a logística para especificar a média $\mu ( t )$ como $\mu ( t ) = \frac{ a \exp{ \{ c t \} } } {1 + b \exp { \{ ct \} }} = \frac{ a} { b + \exp { \{ - ct \} }}$.

Essa expressão é a forma mais simples da logística. Ela pode ser generalizada de várias formas.
